      ...
      <aside className="hidden md:flex w-72 flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 h-screen z-20">
        ...
        <nav className="flex-1 px-4 space-y-1.5">
          ...
          {[
            ...
            { name: "Transactions", href: "#", icon: CreditCard, active: false },
            { name: "System Logs", href: "#", icon: Activity, active: false },
          ].map((item, i) => (
            <Link 
              key={i}
              href={item.href} 
            ...
            >
              <item.icon className={`h-5 w-5 transition-colors ${item.active ? "text-indigo-600" : "text-slate-400 group-hover:text-slate-600"}`} />
          ...
          {[
            ...
            <Link 
              key={i}
              href={item.href} 
              className="flex items-center gap-3.5 px-4 py-3 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-slate-100 transition-all duration-200"
            >
        ...
        <div className="p-6 mt-auto">
          <div className="p-4 rounded-2xl bg-slate-900 dark:bg-indigo-950 text-white relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <Button size="sm" className="w-full h-8 bg-indigo-500 hover:bg-indigo-400 text-xs font-bold rounded-lg border-none shadow-lg shadow-indigo-900/20">
                Upgrade Now
              </Button>
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-20 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl sticky top-0 z-30 flex items-center justify-between px-6 md:px-10">
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            <Button className="hidden sm:flex items-center gap-2 rounded-xl px-5 h-11 bg-slate-900 dark:bg-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-100 font-bold shadow-lg shadow-slate-200 dark:shadow-none transition-all">
